...
There is currently no automated test or lint script. At minimum, run `yarn build` after library changes. Use the root app to exercise source-level component behavior and props. When public exports or integration behavior changes, also build or manually exercise `examples/vue-app`.
